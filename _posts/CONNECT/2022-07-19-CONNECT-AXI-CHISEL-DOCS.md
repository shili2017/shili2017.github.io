...
We may want to run the regression test for all the protocols.
...
To put the network in the chisel wrapper, remove all redundant files and test bench files, and then

```bash
$ cat *.v > network.v
```

Remember to change the corresponding parameters in the chisel wrapper.
